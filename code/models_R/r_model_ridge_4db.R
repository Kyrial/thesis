...
labels_path4 = paste('../../data/redesigned/',bdd4,'/labels_',bdd4,'.csv', sep="")
log_path4 =paste('../../results/',bdd4,'/log_', sep="")

#chargement du fichier
matrix_metrics4 <- do.call(cbind, fromJSON(file = paste(log_path4,'_',bdd4,'_',weight,'_',metric,'_','_BRUTMETRICS','.csv',sep=""),simplify = FALSE))

#si on ne fait pas ça, l'input peut avoir un indice variable
colnames(matrix_metrics4)[2] <- 'input_1'

#idem avec les calculs de complexité
matrix_complexity4 <- do.call(cbind, fromJSON(file = paste(log_path4,'_',bdd4,'_',weight,'_','mean','_','_BRUTMETRICS','.csv',sep=""),simplify = FALSE))
colnames(matrix_complexity4)[2] <- 'input_1'

#passage des matrice en dataframe
df_metrics4 <- as.data.frame(matrix_metrics4, optional = TRUE)
df_complexity4 <- as.data.frame(matrix_complexity4, optional = TRUE)        
#passage en flottants (avant c'était des strings)
df_metrics4 = sapply(df_metrics4, as.numeric)
df_complexity4 = sapply(df_complexity4, as.numeric)

#il faut repasser en df après le sapply
df_metrics4 <- as.data.frame(df_metrics4)
df_complexity4 <- as.data.frame(df_complexity4[,-1])

#changement des noms de colonne pour les uniformiser car les differents weights ont des noms de layers différents
df_metrics4 = plyr::rename(df_metrics4, c("input_1" = "input_1",
                                          'block1_conv1'='conv1_1','block1_conv2'='conv1_2','block1_pool'='pool1',
                                          'block2_conv1'='conv2_1','block2_conv2'='conv2_2','block2_pool'='pool2',
                                          'block3_conv1'='conv3_1','block3_conv2'='conv3_2','block3_conv3'='conv3_3','block3_pool'='pool3',
                                          'block4_conv1'='conv4_1','block4_conv2'='conv4_2','block4_conv3'='conv4_3','block4_pool'='pool4',
                                          ...
                                          'flatten'='flatten','fc1'='fc6_relu','fc2'='fc7_relu'))
#même démarche pour la complexité
df_complexity4 = plyr::rename(df_complexity4, c("input_1" = "input_1_comp",
                                                'block1_conv1'='conv1_1_comp','block1_conv2'='conv1_2_comp','block1_pool'='pool1_comp',
                                                'block2_conv1'='conv2_1_comp','block2_conv2'='conv2_2_comp','block2_pool'='pool2_comp',
                                                'block3_conv1'='conv3_1_comp','block3_conv2'='conv3_2_comp','block3_conv3'='conv3_3_comp','block3_pool'='pool3_comp',
                                                'block4_conv1'='conv4_1_comp','block4_conv2'='conv4_2_comp','block4_conv3'='conv4_3_comp','block4_pool'='pool4_comp',
...
#création d'un dataframe avec la complexité, la sparsité approximée par gini
df4 <- cbind(df_metrics4, df_complexity4)

#Z-transformation (centré réduit)
scaled_df4 <- scale(df4[,-1]) #df[,-1] pour ne pas z transformer la beauté
df4 <- cbind(df4$rate ,scaled_df4) #si on avait pas scaled la beauté il aurait fallu la remettre
#df = scaled_df
df4<- as.data.frame(df4, optional = TRUE)
df4 <- plyr::rename(df4, c("V1" = "rate"))
...
model <- glmnet(x_train, y_train, alpha = 0, lambda = cv_train$lambda.min) #alpha = 0 fait une ridge regression (1 si lasso)
